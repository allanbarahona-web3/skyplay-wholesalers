  ...
  return (
    <div className="wrap">
      <section className="card">
        ...
        <div className="form">
          <div>
            <label htmlFor="user" className="label">Usuario o correo</label>
            ...
            <button className="btn secondary btn-auth" type="button" onClick={() => toast("Función de TOTP simulada")}>🔐 Configurar Google Authenticator</button>
          </div>
          <div>
            <label htmlFor="pass" className="label">Contraseña</label>
            <div className="row">
              <input id="pass" className="input" type={showPass ? "text" : "password"} placeholder="••••••••" autoComplete="current-password" value={pass} onChange={e => setPass(e.target.value)} />
              <button type="button" id="togglePass" className="togglePass" onClick={() => setShowPass(!showPass)}>{showPass ? "🙈" : "👁️"}</button>
            </div>
            <div className="hint">Pon tu contraseña.</div>
          </div>
          <div>
            <div className="meta">
              <label className="label">Código de Google Authenticator</label>
              <span className="muted">(6 dígitos, cambia cada 30 seg)</span>
            </div>
      ...
      <section className="hero">
        <h2 className="hero-title">Catálogo mayorista</h2>
        <p className="hero-desc">Tras validar tu OTP podrás ver el catálogo y comprar en condiciones preferenciales.</p>
      ...
      {resetOpen && (
        <div className="modal open">
          <div className="modal-content">
            ...
            <p>Usa tu email y código de Google Authenticator para restablecer tu contraseña.</p>
            <div className="form">
              <div>
                <label>Email</label>
              ...
              <button className="btn primary" onClick={handleReset}>Cambiar Contraseña</button>
            </div>
          </div>
        </div>
      )}
